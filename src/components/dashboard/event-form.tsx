...
export function EventForm({
  ...
  venues: Option[];
  event?: EventRow;
  /** "drawer" strips the page's Card chrome and pins the actions to a DrawerFooter. */
  layout?: "page" | "drawer";
  /** Drawer only — closes the panel instead of the page-only `router.back()`. */
  onCancel?: () => void;
}) {
      ...
      <Form form={form} onSubmit={save.run}>
        <DrawerBody className="space-y-6">
          <section className="space-y-5">
            <h3 className="text-sm font-semibold text-ink">Basics</h3>
            {basicsFields}
          </section>
          ...
          <section className="space-y-5">
            <h3 className="text-sm font-semibold text-ink">Where</h3>
            {whereFields}
          </section>
  ...
  return (
    <Form form={form} onSubmit={save.run} className="space-y-5">
      <Card>
        ...
        <CardBody className="space-y-5">{basicsFields}</CardBody>
      </Card>
      ...
      <Card>
        ...
        <CardBody className="space-y-5">{whereFields}</CardBody>
      </Card>
